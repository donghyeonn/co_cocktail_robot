...
from co_cocktail_robot.object_detection.realsense import ImgNode
from co_cocktail_robot.object_detection.yolo import YoloModel


package_path = get_package_share_directory("co_cocktail_robot")
MODEL_NAME = "best.pt"
# MODEL_PATH = f"../resource/{MODEL_NAME}"
...
class ObjectDetectionNode(Node):
    ...
    def _compute_position(self, target):
        ...
        # Target 별로 계산
        if target == 'lime':
            cx = int((box[0] + box[2]) / 2) -2
            cy = int((box[1] + box[3]) / 2)
            self.get_logger().info(f"Target: {target}, cx: {cx}, cy: {cy}")
        elif target == 'cherry':
            cx = int((box[0] + box[2]) / 2) 
            cy = int((box[1] + box[3]) / 2)-3
            self.get_logger().info(f"Target: {target}, cx: {cx}, cy: {cy}")

        elif target == 'red_juice':
            cx = int((box[0] + box[2]) / 2)
            cy = int((box[1] + box[3]) / 2) + 10
        elif target == 'blue_juice':
            cx = int((box[0] + box[2]) / 2)
            cy = int((box[1] + box[3]) / 2)
            cz = self._get_depth(cx, cy)
            if cz is None:
                self.get_logger().warn("Depth out of range for blue_juice.")
                return 0.0, 0.0, 0.0
            cz -= 30  # Depth에 추가 보정
            self.get_logger().info(f"Target: blue_juice, cz: {cz}")
            return cx, cy, cz
        else:
