...
# print("설정한 모델 경로: ", os.path.join(PACKAGE_PATH, "resource", YOLO_MODEL_FILENAME))
...
class YoloModel:
    def __init__(self):
        ...
        self.code_to_yolo = {
            "blue-juice": "blue_juice",
            "red-juice": "red_juice",
            "green-juice": "green_juice",
            "tequila": "tequila",
            "shaker": "shaker",
            "cherry": "cherry",
            "lime": "lime",
        ...
        with open(YOLO_JSON_PATH, "r", encoding="utf-8") as file:
            ...
            # self.code_to_yolo = json.load(file)
            # self.yolo_name_to_idx = {v: i for i, v in enumerate(self.model.names)}
    ...
    def get_best_detection(self, img_node, target):
        yolo_name = self.code_to_yolo[target]
        yolo_name_to_idx = {v: i for i, v in self.model.names.items()}
        label_id = yolo_name_to_idx[yolo_name]
    ...
    def _aggregate_detections(self, results, confidence_threshold=0.6, iou_threshold=0.7):
        """
