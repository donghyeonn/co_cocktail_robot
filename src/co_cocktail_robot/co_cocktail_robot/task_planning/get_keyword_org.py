...
is_laod = load_dotenv(dotenv_path=os.path.join(f"{package_path}/share/co_cocktail_robot/resource/.env"))
...
class GetKeyword(Node):
    def __init__(self):
        ...
        prompt_content = """
            ...
            - 입력: "셰이커 좀 섞어줘"  
            출력: [['X', 'close', 'X'], ['tumbler', 'shake', 'X'], ['X', 'open', 'X'], ['tumbler', 'take', 'X'], ['X', 'drop', 'pos4']]
        ...
        self.get_keyword_srv = self.create_service(
            GetKeywordList, "/get_keyword", self.get_keyword
        )
    ...
    def get_keyword(self, request, response):  # 요청과 응답 객체를 받아야 함
        ...
        response.keywords = []
        for group in eval(keyword):
            kg = KeywordGroup()
            print(f"group:{group}")
            print(f"type(group):{type(group)}")
            kg.items = group  # 리스트 of str
            response.keywords.append(kg)
        self.get_logger().warn(f"Results {eval(keyword)}")
        self.get_logger().warn(f"Results type {type(eval(keyword))}")
        # 응답 객체 설정
        # response.keywords = eval(keyword)
